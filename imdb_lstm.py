...
results_df.to_csv(
    os.path.join(OUTPUT_DIR, "lstm_results.csv"),
    index=False
)
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "lstm_accuracy.png"), dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "lstm_loss.png"), dpi=300)
plt.close()
